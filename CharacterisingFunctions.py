...
def compare(freqdictA,freqdictB,indicatordict):
    #compare two frequency distributions (as part of bootstrap)
    #freqdictA is a dictionary
    #freqdictB is a dictionary
    
    sizeA=sum(freqdictA.values())
    sizeB=sum(freqdictB.values())
    
  #  logging.info("{},{}".format(sizeA,sizeB))
    
    for i,(word,freqA) in enumerate(freqdictA.items()):
        freqB=freqdictB.get(word,0)
        
        probA=freqA/sizeA
        probB=freqB/sizeB
       # logging.info("{}: ({},{})".format(word,probA,probB))
        if probA>probB:
            indicatordict[word]=indicatordict.get(word,0)+1
    return indicatordict


def checkconvergence(newdict,cache,N,t=0.9):

    #check whether the candidates generated by bootstrapping are converging
    #N is the number of bootstraps done so far
    ...
    for term in cache.keys():
        absdiffs.append(abs(cache[term]-newcache.get(term,0)))
...
def bootstrap(labelset,worddocdict,size=0):
    ...
    for word in worddocdict.keys():
        for i,label in enumerate(sample):
            value = worddocdict[word].get(label,0)
            if value>0:
...
def bootstrap_compare(corpusA,Bdist,interval=100,repeats=10,ftype="termfreq",balanced=False,p=0.999,size=0):
    
    sizeB=Bdist[0]
    freqB=makedict(Bdist[1])
    ...
    for j in range(0,repeats):
        ...
        #starttime=time()
        termfreqA,docfreqA=bootstrap(corpusA[0],corpusA[1],size=bootstrapsize)
        if ftype=="docfreq":
            indicatordict=compare(docfreqA,freqB,indicatordict)
        else:
            indicatordict=compare(termfreqA,freqB,indicatordict)
       
        #timetaken=time()-starttime
        #logging.info("Time taken: {}".format(timetaken))
        if (j+1)%interval==0:
            logging.info("Bootstrapping corpusA repetition {}".format(j))

            cacheddict,stop=checkconvergence(indicatordict,cacheddict,j+1)
            if stop:
                N=j+1
                break
